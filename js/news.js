async function loadGallery() {
    ...
    try {
        // Chiamata all'API Apps Script per la galleria
        ...
        if (result.status !== "success" || !result.data || result.data.length === 0) {
            if (container) container.innerHTML = '<p class="text-center text-muted">Nessuna immagine trovata nella cartella.</p>';
            return;
        }
        ...
        if (loading) loading.style.display = "none";
        if (container) container.innerHTML = '<p class="text-center text-danger">Impossibile caricare le immagini al momento.</p>';
    }
}
...
$(document).ready(function() {
    ...
    fetch("https://script.google.com/macros/s/AKfycbxEfogdhjb9y7WidXORRd8PhuWtw0tedKLpSUp3SELaF2bHB9oXrO790lu5otopjVvJ/exec")
        .then(response => response.json())
        .then(data => {
            let parsedData = (typeof data === "string") ? JSON.parse(data) : data;
            console.log("Dati ricevuti da Apps Script (iFrame):", parsedData);
        ...
        .catch(error => console.error("Errore durante il caricamento degli iFrame:", error));

    // 2. Caricamento Locandine Eventi
    ...
    // 3. Avvio della galleria immagini
    loadGallery();
});
